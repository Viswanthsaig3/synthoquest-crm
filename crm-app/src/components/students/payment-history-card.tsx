...
import { getStudentPaymentSummary, createStudentPayment, deleteStudentPayment } from '@/lib/api/students'
...
import type { StudentPaymentSummary, StudentPayment } from '@/types/student'
import AddStudentPaymentModal from './add-payment-modal'

interface StudentPaymentHistoryCardProps {
  studentId: string
  enrollmentId: string
  onPaymentUpdate?: () => void
}

export default function StudentPaymentHistoryCard({ studentId, enrollmentId, onPaymentUpdate }: StudentPaymentHistoryCardProps) {
  const { user } = useAuth()
  ...
  const canManagePayments = user ? canManageStudentPayments(user) : false
  ...
  const loadPaymentSummary = async () => {
    try {
      setLoading(true)
      const res = await getStudentPaymentSummary(studentId, enrollmentId)
      setSummary(res.data)
    } catch (error) {
  ...
  const handleAddPayment = async (data: { amount: number; paymentMethod: string; paymentDate?: string; receiptNumber?: string; notes?: string }) => {
    try {
      await createStudentPayment(studentId, enrollmentId, {
        amount: data.amount,
        paymentMethod: data.paymentMethod as StudentPayment['paymentMethod'],
  ...
  const handleDeletePayment = async (paymentId: string) => {
    ...
    try {
      await deleteStudentPayment(studentId, enrollmentId, paymentId)
      toast({
